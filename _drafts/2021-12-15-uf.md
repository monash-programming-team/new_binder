...
As such, Union Find is useful in any problem where connections are incrementally being added to some problem, and along the way you need to query what vertices are connected.
...
This means that `find` is log(n), meaning both `merge` and `connected` are also log(n).
